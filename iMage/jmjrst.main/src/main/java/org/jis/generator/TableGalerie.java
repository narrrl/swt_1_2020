  ...
  public TableGalerie(Main main) {
    ...
    try {
      ...
      m.jOutputDoc.insertString(
          m.jOutputDoc.getLength(), images.length + m.mes.getString("Generator.28") + images[0].getParent()
              + m.mes.getString("Generator.29") + o.getQuality() + m.mes.getString("Generator.30") + ls + ls,
          m.outputAtr);
    ...
    Thread t = new Thread() {
      public void run() {
        ...
        for (int i = 0; i < images.length; i++) {
          ...
          sb.append("      <td><a href=\"medium/image" + i + ".html\" title=\"" + titel + "\"><img src=\"small/"
              + out_s.getName() + "\" alt=\"" + titel + "\" /></a></td>\n");
          ...
          if (count == x || i == (max - 1)) {
            ...
            sb.append("    <tr>\n");
            sb.append("     <td colspan=\"" + o.getGallerieWidth() + "\" style=\"width: 100%; background-color: rgb("
                + o.getBackground_r() + "," + o.getBackground_g() + "," + o.getBackground_b()
                + "); vertical-align: top;\">\n");
            if (lastPage > 1) {
              if (page == 1)
                sb.append("      <a href=\"index" + (page + 1) + ".html\" class=\"next\">&gt;&gt;</a>\n");
                ...
                if (page == 2)
                  ...
                  sb.append("      <div class=\"next\"><a href=\"index" + (page - 1)
                      + ".html\" class=\"next\">&lt;&lt;</a>&nbsp;|&nbsp;<a href=\"index" + (page + 1)
              ...
              if ((i + 1) == max) {
                try {
                  ...
                  m.jOutputDoc.insertString(m.jOutputDoc.getLength(),
                      ls + images.length + m.mes.getString("Generator.44") + o.getOutput_dir_gallerie()
                          + m.mes.getString("Generator.45") + ls,
                      m.readyAtr);
                  m.text.setCaretPosition(m.jOutputDoc.getLength());
                } catch (Exception e) {
        ...
        try {
          m.jOutputDoc.insertString(m.jOutputDoc.getLength(), ls + images.length + m.mes.getString("Generator.44")
              + o.getOutput_dir_gallerie() + m.mes.getString("Generator.45") + ls, m.readyAtr);
          m.text.setCaretPosition(m.jOutputDoc.getLength());
        } catch (Exception e) {
